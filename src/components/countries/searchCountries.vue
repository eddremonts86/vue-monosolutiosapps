<template>
  <div class="mx-auto search_box radius7 pa-5">
      ...
      <v-flex class="xs8 sm10">
        ...
        >
          <template class="custom_search_text" v-slot:name>
            <v-icon class="custom_search_text" style="vertical-align: middle">search</v-icon>Search by country name
          </template>
          <template v-slot:selection="data">
            <v-chip :input-value="data.selected" @click:close="remove()" close>
              <img :src="data.item.flag" class="mx-1 radius4" height="14px" width="20px" />
              {{ data.item.name }}
            </v-chip>
          </template>
          ...
          <v-card-text class="px-3 pt-3 pb-0">
            <v-form lazy-validation ref="form">
              <v-layout wrap>
                <v-flex xs2>
                  <v-switch
                    @click="elementsChanges('capitalSwitch')"
                    v-model="showElements['capitalSwitch']"
                  ></v-switch>
                </v-flex>
                <v-flex xs10>
                  <v-autocomplete
                  ...
                  >
                    <template class="custom_search_text" v-slot:capital>
                      <v-icon class="custom_search_text" style="vertical-align: middle">search</v-icon>Search by country capital
                    </template>
                    <template v-slot:selection="data">
                      <v-chip
                        :input-value="data.selected"
                        @click:close="serachItems['capital'] = 'none'"
                        close
                      >
                        <img :src="data.item.flag" class="mx-1 radius7" height="18px" width="18px" />
                    ...
                    <template v-slot:item="data">
                      <template>
                        <img :src="data.item.flag" class="mx-1 radius7" height="18px" width="18px" />
                        <v-list-item-content v-text="data.item.capital" />
                      </template>
                    </template>
                  </v-autocomplete>
                </v-flex>
              ...
              <v-layout wrap>
                <v-flex xs2>
                  <v-switch
                    @click="elementsChanges('langSwitch')"
                    v-model="showElements['langSwitch']"
                  ></v-switch>
                </v-flex>
                <v-flex xs10>
                  <v-autocomplete
                  ...
                  >
                    <template class="custom_search_text" v-slot:name>
                      <v-icon class="custom_search_text" style="vertical-align: middle">search</v-icon>Search by language
                    </template>
                    <template v-slot:selection="data">
                      <v-chip
                        :input-value="data.selected"
                        @click:close="serachItems['language'] = 'none'"
                        close
                    ...
                    <template v-slot:item="data">
                      <template>
                        <v-list-item-content v-text="data.item.name" />
                        <v-spacer></v-spacer>
                        <span class="grey darken-1 pa-2 radius7">
                          Native : {{
              ...
              <v-layout wrap>
                <v-flex xs2>
                  <v-switch
                    @click="elementsChanges('currencySwitch')"
                    v-model="showElements['currencySwitch']"
                  ></v-switch>
                </v-flex>
                <v-flex xs10>
                  <v-autocomplete
                  ...
                  >
                    <template class="custom_search_text" v-slot:name>
                      <v-icon class="custom_search_text" style="vertical-align: middle">search</v-icon>Search by courrencies
                    </template>
                    <template v-slot:selection="data">
                      <v-chip
                        :input-value="data.selected"
                        @click:close="serachItems['currency'] = 'none'"
                        close>
                        {{ data.item.name }}  <span class="grey darken-1 px-2 ml-3 radius7">{{ data.item.symbol }}</span>
                      </v-chip>
                    </template>
                    <template v-slot:item="data">
                      <template>
                        <v-list-item-content v-text="data.item.name" />
                        <v-spacer></v-spacer>
                        <span class="grey darken-1 pa-2 radius7">{{ data.item.symbol }}</span>          
                      </template>
                    </template>
                  </v-autocomplete>
                </v-flex>
    ...
    <v-dialog v-model="getWindowsStatus" width="60%" persistent>
      <country />
    </v-dialog>
  </div>
</template>
